...
const { displayUsage } = require('./myapp');
  ...
  tokenApp: async function() {
    try {
      ...
      switch (myArgs[0]) {
        ...
        case '--upd':
          if (myArgs.length < 4) {
            console.log('Invalid syntax. Usage: node myapp token --upd [field] [username] [value]');
          } else {
            await updateToken(myArgs[1], myArgs[2], myArgs[3]);
          }
          break;
        case '--search':
          if (myArgs.length < 3) {
            console.log('Invalid syntax. Usage: node myapp token --search [field] [value]');
          } else {
            await fetchToken(myArgs[1], myArgs[2]);
          }
          break;
        default:
          console.log('Invalid option. Usage: node myapp token [--list | --new username | --upd field username value | --search field value]');
      }
    } catch (error) {
