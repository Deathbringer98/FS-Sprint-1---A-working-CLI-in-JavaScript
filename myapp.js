...
// token options
  ...
  .option('--new <username>', 'Generate a token for a given username, saves tokens to the json file')
  .option('--upd <field> <username> <value>', 'Update the json entry with a new value (field can be "p" for phone or "e" for email)')
  .option('--search <field> <value>', 'Fetch a token for a given value (field can be "u" for username, "e" for email, or "p" for phone)')
  .action(async (options) => {
    if (options.help) {
      ...
      console.log('Please provide a valid option for the token command. Use --help for assistance.');
    }
  });
...
async function generateToken(username) {
  try {
    // Generate token logic goes here
...
// Place this block before app.listen()
const express = require('express');
const app = express();

// This line is required to parse the body of the POST request
app.use(express.urlencoded({ extended: true }));

app.post('/submit_registration', (req, res) => {
  console.log(req.body); // This will log the data submitted by the form
